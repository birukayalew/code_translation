...
c2saferustv2_df = pd.read_excel('warnings_per_program.xlsx', sheet_name='Summary', skiprows=26, nrows=7, index_col='Program').fillna(0)
human_written_df = pd.read_excel('warnings_per_program.xlsx', sheet_name='Summary', skiprows=38, nrows=7, index_col='Program').fillna(0)
...
c2saferustv2_norm = normalize_df(c2saferust_df)
...
for category in categories:
    ...
    data4 = c2saferustv2_norm[category]

    # Skip categories with identical values across groups.
    if (data1.nunique() == 1 and data2.nunique() == 1 and data3.nunique() == 1 and data4.nunique() and 
